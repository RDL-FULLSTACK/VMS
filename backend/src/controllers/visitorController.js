...
// 🔹 Search Visitor by Last 4 Digits of ID
exports.searchVisitorByLastFourDigits = async (req, res) => {
  try {
    ...
    if (!lastFourDigits) {
      return res.status(400).json({
        success: false,
        message: "Please provide the last 4 digits of the visitor ID",
      });
    }

    if (!/^[0-9a-fA-F]{4}$/.test(lastFourDigits)) {
      return res.status(400).json({
        success: false,
        message: "Last 4 digits must be 4 hexadecimal characters (0-9, a-f)",
      });
    }

    // Convert lastFourDigits to lowercase to ensure consistency
    const normalizedLastFourDigits = lastFourDigits.toLowerCase();

    // Use aggregation to convert _id to string and match the last 4 digits
    const visitors = await Visitor.aggregate([
      // Add a new field that converts _id to a string
      {
        $addFields: {
          idAsString: { $toString: "$_id" },
        },
      },
      // Match documents where the idAsString ends with the provided last 4 digits
      {
        $match: {
          idAsString: { $regex: `${normalizedLastFourDigits}$` },
        },
      },
      // Limit to 1 result (equivalent to findOne)
      { $limit: 1 },
    ]);

    // Check if a visitor was found
    ...
    // Since aggregation returns raw documents, we fetch the full visitor document using findById
    const visitor = await Visitor.findById(visitors[0]._id);
    ...
    res.status(500).json({
      success: false,
      message: "Server error while searching for visitor",
      error: error.message,
    });
  }
};
