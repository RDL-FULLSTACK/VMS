...
function Home() {
  ...
  // Combine notifications into a single string with separators for marquee effect
  const notificationText = notifications.length > 0
    ? notifications.join("    •    ") // Add a separator between messages
    : "";

  return (
    <>
      <Navbar />
      {/* Floating Notification Marquee */}
      {notifications.length > 0 && (
        <Box
          sx={{
            position: "sticky",
            top: 64, // Default MUI Navbar height
            left: 0,
            right: 0,
            bgcolor: "#1976d2",
            color: "white",
