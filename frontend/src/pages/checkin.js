...
//             variant="contained"
//             color="primary"
//             onClick={() => navigate("/employees")} // Updated to navigate to /employees
//             sx={{ height: "48px", minWidth: "150px" }}
//           >
//             Employee
//           </Button>
...
const Checkin = () => {
  ...
  const [photo, setPhoto] = useState(null);
  const [photoPreview, setPhotoPreview] = useState(null);
  const [employees, setEmployees] = useState([]); // Renamed from 'hosts' to 'employees'
  const [formData, setFormData] = useState({
    fullName: "",
    email: "",
    phoneNumber: "",
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        const data = await response.json();
        setEmployees(data); // Fetch all employees, no role filter
      } catch (error) {
  ...
  const handleInputChange = async (field, value) => {
    ...
    // Reset personToVisit if department changes and current selection is invalid
